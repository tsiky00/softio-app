...
$(document).ready(function () {
    ...
    for (var i = 0; i < users.length; i++) {
      ...
      <tr>
          ...
          <td class="text-center">
              <a class="btn btn-primary edit-btn" data-id="${user.idService}" title="Modifier">
                  <i class="fa fa-edit"></i>
              </a>
              <a class="btn btn-danger delete-btn" data-id="${user.idService}" title="Supprimer">
  ...
  $(document).on("click", ".edit-btn", function () {
    ...
    $.ajax({
      url: "service/get-service/" + userId,
      method: "GET",
      dataType: "json",
      success: function (res) {
        if (res.status === "success") {
          const u = res.data;
          $("#edit-id").val(u.idService);
